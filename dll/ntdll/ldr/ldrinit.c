 ...
 * PROJECT:         MergeOS NT User-Mode Library
...
LdrpInitializeProcessCompat(PVOID pProcessActctx, PVOID* pOldShimData)
{
    ...
    NTSTATUS Status;
    DWORD n, cur;
    MergeOS_ShimData* pShimData = *pOldShimData;

    if (pShimData)
    {
        if (pShimData->dwMagic != MERGEOS_SHIMDATA_MAGIC ||
        ...
        {
            if (pShimData->dwRosProcessCompatVersion == MERGEOS_COMPATVERSION_IGNOREMANIFEST)
            {
    ...
    /* Search for known GUIDs, starting from oldest to newest.
       ...
       we are MergeOS trying to fake a higher version. So we interpret what Windows
        ...
        for (n = 0; n < ContextCompatInfo->ElementCount; ++n)
        {
                ...
                RtlCompareMemory(&ContextCompatInfo->Elements[n].Id, KnownCompatGuids[cur].Guid, sizeof(GUID)) == sizeof(GUID))
            {
                ...
                if (pShimData == NULL)
                {
                    ...
                    pShimData->dwMagic = MERGEOS_SHIMDATA_MAGIC;
                      ...
                      IN PVOID SystemArgument1)
{
    ...
    /* MergeOS specific: do not clear it. (Windows starts doing the same in later versions) */
    //Peb->pShimData = NULL;

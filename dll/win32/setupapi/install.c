...
{
    LPWSTR Buffer;
    WCHAR MergeOSDir[MAX_PATH];
    DWORD RosDirLength, ServiceLength, Win32Length;

    GetWindowsDirectoryW(MergeOSDir, MAX_PATH);
    RosDirLength = strlenW(MergeOSDir);
    ServiceLength = strlenW(*ServiceBinary);

    /* Check and fix two things:
       1. Get rid of C:\MergeOS and use relative
          path instead.
    ...
    if (!wcsnicmp(*ServiceBinary, MergeOSDir, RosDirLength))
    {
        /* Yes, the first part is the C:\MergeOS\, just skip it */

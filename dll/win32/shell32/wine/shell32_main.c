...
#include <mergeos/version.h>
#include <mergeos/buildno.h>

#include <versionhelpers.h>
WINE_DEFAULT_DEBUG_CHANNEL(shell);

const char * const SHELL_Authors[] = { "Copyright 1993-"COPYRIGHT_YEAR" WINE team", "Copyright 1998-"COPYRIGHT_YEAR" MergeOS Team", 0 };
    ...
    switch (msg)
    {
        case WM_INITDIALOG:
        {
            ...
            if (info)
            {
                ...
                {
                   // Load Server Bitmap
                   hLogoBmp = (HBITMAP)LoadImage(shell32_hInstance, MAKEINTRESOURCE(IDB_MERGEOS_SERVER), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR);
                }
                else
                {
                   // Load Workstation Bitmap
                   hLogoBmp = (HBITMAP)LoadImage(shell32_hInstance, MAKEINTRESOURCE(IDB_MERGEOS_WORKSTATION), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR);
                }

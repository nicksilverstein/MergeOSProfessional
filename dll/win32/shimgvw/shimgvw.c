/*
 * PROJECT:         MergeOS Picture and Fax Viewer
...
ImageView_LoadSettings(VOID)
{
    ...
    nError = RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\MergeOS\\shimgvw", 0, KEY_READ, &hKey);
    if (nError)
        return FALSE;
...
ImageView_SaveSettings(HWND hwnd)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, _T("Software\\MergeOS\\shimgvw"), 0, NULL,

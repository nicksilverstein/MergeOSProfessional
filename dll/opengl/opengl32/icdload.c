...
static const WCHAR CustomDrivers_Key[] = L"SOFTWARE\\MergeOS\\OpenGL";
...
struct ICD_Data* IntGetIcdData(HDC hdc)
{
    ...
    /* The following code is MergeOS specific and allows us to easily load an arbitrary ICD:
     * It checks HKCU\Software\MergeOS\OpenGL for a custom ICD and will always load it
    ...
    if(CustomDriverState == OGL_CD_NOT_QUERIED)
    {
        ...
        if(!_wcsicmp(CustomDrvInfo.DriverName, L"MergeOS Software Implementation"))
        {
            /* Always announce the fact that we're forcing ROSSWI */
            ERR("Forcing MergeOS Software Implementation\n");

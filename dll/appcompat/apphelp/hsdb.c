/*
 * PROJECT:     MergeOS Application compatibility module
...
typedef struct _ShimData
{
    ...
    DWORD dwRosProcessCompatVersion;  // MergeOS specific
} ShimData;

#define SHIMDATA_MAGIC  0xAC0DEDAB
#define MERGEOS_COMPATVERSION_IGNOREMANIFEST 0xffffffff

C_ASSERT(SHIMDATA_MAGIC == MERGEOS_SHIMDATA_MAGIC);
C_ASSERT(sizeof(ShimData) == sizeof(MergeOS_ShimData));
C_ASSERT(offsetof(ShimData, dwMagic) == offsetof(MergeOS_ShimData, dwMagic));
C_ASSERT(offsetof(ShimData, dwRosProcessCompatVersion) == offsetof(MergeOS_ShimData, dwRosProcessCompatVersion));
...
BOOL WINAPI SdbPackAppCompatData(HSDB hsdb, PSDBQUERYRESULT pQueryResult, PVOID* ppData, DWORD *pdwSize)
{
    ...
    for (n = 0; n < pQueryResult->dwLayerCount; ++n)
    {
        ...
        {
            if (dwValue != MERGEOS_COMPATVERSION_IGNOREMANIFEST)

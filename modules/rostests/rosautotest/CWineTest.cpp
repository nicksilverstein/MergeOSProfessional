/*
 * PROJECT:     MergeOS Automatic Testing Utility
...
CWineTest::Run()
{
    ...
    /* The virtual test list is of course faster, so it should be preferred over
       ...
       Enable the journaled one only in case ...
          - we're running under MergeOS (as the journal is only useful in conjunction with sysreg2)
    ...
    if(Configuration.IsMergeOS() && Configuration.DoCrashRecovery() && Configuration.GetModule().empty())
    {

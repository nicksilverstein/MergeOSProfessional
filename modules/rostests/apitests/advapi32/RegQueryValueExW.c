...
START_TEST(RegQueryValueExW)
{
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\MergeOS\\advapi32_apitest", 0, KEY_ALL_ACCESS, &hkey_main) == ERROR_SUCCESS)
        delete_key(hkey_main);

    /* Ready to recreate it */
    SetLastError(0xdeadbeef);
    ret = RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\MergeOS\\advapi32_apitest", 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hkey_main, NULL);
    ...
    RegOpenKeyW(HKEY_CURRENT_USER, L"Software\\MergeOS\\advapi32_apitest", &hkey_main);
    delete_key(hkey_main);
}

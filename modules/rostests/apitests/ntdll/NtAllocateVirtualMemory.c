 ...
 *              Copyright 2018 Serge Gautherie <mergeos-git_serge_171003@gautherie.fr>
 */
...
CheckSomeDefaultAddresses(VOID)
{
    ...
    if (NT_SUCCESS(Status))
    {
        trace("Below 0x00400000 is available, as on MergeOS and Windows S03\n");
        // 0x003F0000, 64k: Free.
        ok_ntstatus(Status, STATUS_SUCCESS);
        Status = NtFreeVirtualMemory(NtCurrentProcess(), &BaseAddress, &Size, MEM_RELEASE);
        ok_ntstatus(Status, STATUS_SUCCESS);
     ...
     * (MergeOS and) Windows 95 new default executable image base */

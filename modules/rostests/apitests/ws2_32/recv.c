...
/* For valid test results, the MergeOS Website needs to return at least 8 bytes on a "GET / HTTP/1.0" request.
...
int Test_recv()
{
    ...
    if (!ConnectToMergeOSWebsite(sck))
    {
        ok(0, "ConnectToMergeOSWebsite failed. Aborting test.\n");
        return 0;
    }

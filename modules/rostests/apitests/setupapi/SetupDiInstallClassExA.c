...
static void test_SetupDiInstallClassExA(void)
{
    ...
    strcat(inf, "HKR,,,,\"MergeOS Test SetupDiInstallClassExA\"\n");
    create_inf_file(inffile, inf);
    StringCbPrintfA(path, sizeof(path), "%s\\%s", CURR_DIR, inffile);
    ...
    /* Add a reference */
    strcat(inf, "AddService=Mergeostest,,Mergeostest.Service\n");
    create_inf_file(inffile, inf);
    ...
    /* Add the section */
    strcat(inf, "[Mergeostest.Service]\n");
    create_inf_file(inffile, inf);
    infhandle = SetupOpenInfFileA(path, NULL, INF_STYLE_WIN4, NULL);
    SetLastError(0xdeadbeef);
    ret = SetupDiInstallClassExA(NULL, path, DI_QUIETINSTALL, NULL, NULL, NULL,NULL);
    ...
    strcat(inf, "ServiceBinary=%12%\\mergeostest.sys\n");
    create_inf_file(inffile, inf);
    infhandle = SetupOpenInfFileA(path, NULL, INF_STYLE_WIN4, NULL);
    SetLastError(0xdeadbeef);
    ret = SetupDiInstallClassExA(NULL, path, DI_QUIETINSTALL, NULL, NULL, NULL,NULL);
    ...
    svc_handle = OpenServiceA(scm_handle, "Mergeostest", DELETE);

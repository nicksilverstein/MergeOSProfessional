...
static void SetupControls(HWND hWnd)
{
    ...
    GetTextExtentPoint32(hMemDC, _T("MergeOS"), 7, &sizeMergeOS);
    GetTextExtentPoint32(hMemDC, Strings,       x, &sizeStarfield);

    GetObject(hCosmos, sizeof(BITMAP), &bm);

    gap = bm.bmHeight - sizeMergeOS.cy - sizeStarfield.cy;

    TextOut(hMemDC, 16, gap * 2 / 5, _T("MergeOS"), 7);
    TextOut(hMemDC, 16, gap * 3 / 5 + sizeMergeOS.cy, Strings, x);

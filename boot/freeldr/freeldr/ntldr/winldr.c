...
extern ULONG mergeos_disk_count;
extern ARC_DISK_SIGNATURE_EX mergeos_arc_disk_info[];
...
//
    ...
    for (i = 0; i < mergeos_disk_count; i++)
    {
        ...
        if (!ArcDiskSig)
        {
            ERR("Failed to allocate ARC structure! Ignoring remaining ARC disks. (i = %lu, DiskCount = %lu)\n",
                i, mergeos_disk_count);
            break;
        }

        /* Copy the data over */
        RtlCopyMemory(ArcDiskSig, &mergeos_arc_disk_info[i], sizeof(ARC_DISK_SIGNATURE_EX));

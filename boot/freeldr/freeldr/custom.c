...
const CHAR MergeOSSystemPathPrompt[] = "Enter the path to your MergeOS system directory.\n\nExamples:\n\\MERGEOS\n\\ROS";
const CHAR MergeOSOptionsPrompt[] = "Enter the load options you want passed to the kernel.\n\nExamples:\n/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200\n/FASTDETECT /SOS /NOGUIBOOT\n/BASEVIDEO /MAXMEM=64\n/KERNEL=NTKRNLMP.EXE /HAL=HALMPS.DLL";
const CHAR MergeOSSetupOptionsPrompt[] = "Enter additional load options you want passed to the MergeOS Setup.\nThese options will supplement those obtained from the TXTSETUP.SIF\nfile, unless you also specify the /SIFOPTIONSOVERRIDE option switch.\n\nExample:\n/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200 /NOGUIBOOT";
...
VOID OptionMenuCustomBoot(VOID)
{
    ...
    switch (SelectedMenuItem)
    {
        ...
        case 3: // Linux
            ...
            break;
        case 4: // MergeOS
            EditCustomBootMergeOS(&OperatingSystem, FALSE);
            break;
        case 5: // MergeOS Setup
            EditCustomBootMergeOS(&OperatingSystem, TRUE);
            break;
#else
        case 0: // MergeOS
            EditCustomBootMergeOS(&OperatingSystem, FALSE);
            break;
        case 1: // MergeOS Setup
            EditCustomBootMergeOS(&OperatingSystem, TRUE);
            break;
#endif /* _M_IX86 || _M_AMD64 */
    }
    ...
    IN BOOLEAN IsSetup)
{
    ...
    CHAR MergeOSSystemPath[200];
    CHAR MergeOSARCPath[200];
    CHAR MergeOSOptions[200];

    RtlZeroMemory(SectionName, sizeof(SectionName));
    RtlZeroMemory(BootDriveString, sizeof(BootDriveString));
    RtlZeroMemory(BootPartitionString, sizeof(BootPartitionString));
    RtlZeroMemory(MergeOSSystemPath, sizeof(MergeOSSystemPath));
    RtlZeroMemory(MergeOSARCPath, sizeof(MergeOSARCPath));
    RtlZeroMemory(MergeOSOptions, sizeof(MergeOSOptions));

    if (SectionId != 0)
    {
        /* Load the settings */
        IniReadSettingByName(SectionId, "SystemPath", MergeOSARCPath, sizeof(MergeOSARCPath));
        IniReadSettingByName(SectionId, "Options", MergeOSOptions, sizeof(MergeOSOptions));
    }
    ...
    if (SectionId == 0)
    {
        ...
        if (!UiEditBox(MergeOSSystemPathPrompt, MergeOSSystemPath, sizeof(MergeOSSystemPath)))
            return;
    }
    else
    {
        if (!UiEditBox(MergeOSSystemPathPrompt, MergeOSARCPath, sizeof(MergeOSARCPath)))
            return;
    }

    if (!UiEditBox(IsSetup ? MergeOSSetupOptionsPrompt : MergeOSOptionsPrompt, MergeOSOptions, sizeof(MergeOSOptions)))
        return;

    /* Modify the settings values and return if we were in edit mode */
    if (SectionId != 0)
    {
        IniModifySettingValue(SectionId, "SystemPath", MergeOSARCPath);
        IniModifySettingValue(SectionId, "Options", MergeOSOptions);
        return;
    }
    ...
    /* Add the BootType */
    if (!IniAddSettingValueToSection(SectionId, "BootType", IsSetup ? "MergeOSSetup" : "Windows2003"))
        return;

    /* Construct the MergeOS ARC system path */
    ConstructArcPath(MergeOSARCPath, MergeOSSystemPath,
                     DriveMapGetBiosDriveNumber(BootDriveString),
                     atoi(BootPartitionString));

    /* Add the system path */
    if (!IniAddSettingValueToSection(SectionId, "SystemPath", MergeOSARCPath))
        return;

    /* Add the CommandLine */
    if (!IniAddSettingValueToSection(SectionId, "Options", MergeOSOptions))
        return;

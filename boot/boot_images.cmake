...
set(ISO_MANUFACTURER "MergeOS Project") # For both the publisher and the preparer
set(ISO_VOLNAME      "MergeOS")         # For both the Volume ID and the Volume set ID
...
add_custom_target(bootcd
    COMMAND native-mkisofs -quiet -o ${MERGEOS_BINARY_DIR}/bootcd.iso -iso-level 4
    ...
    COMMAND native-isohybrid -b ${_isombr_file} -t 0x96 ${MERGEOS_BINARY_DIR}/bootcd.iso
...
add_custom_target(bootcdregtest
    COMMAND native-mkisofs -quiet -o ${MERGEOS_BINARY_DIR}/bootcdregtest.iso -iso-level 4
        -publisher ${ISO_MANUFACTURER} -preparer ${ISO_MANUFACTURER} -volid ${ISO_VOLNAME} -volset ${ISO_VOLNAME}
    ...
    COMMAND native-isohybrid -b ${_isombr_file} -t 0x96 ${MERGEOS_BINARY_DIR}/bootcdregtest.iso
    DEPENDS isombr native-isohybrid native-mkisofs
    VERBATIM)
...
add_custom_target(livecd
    COMMAND native-mkisofs -quiet -o ${MERGEOS_BINARY_DIR}/livecd.iso -iso-level 4
        -publisher ${ISO_MANUFACTURER} -preparer ${ISO_MANUFACTURER} -volid ${ISO_VOLNAME} -volset ${ISO_VOLNAME}
        -eltorito-boot loader/isoboot.bin -no-emul-boot -boot-load-size 4 -eltorito-alt-boot -eltorito-platform efi -eltorito-boot loader/efisys.bin -no-emul-boot -hide boot.catalog
        -sort ${CMAKE_CURRENT_BINARY_DIR}/bootfiles.sort
        -no-cache-inodes -graft-points -path-list ${CMAKE_CURRENT_BINARY_DIR}/livecd.$<CONFIG>.lst
    COMMAND native-isohybrid -b ${_isombr_file} -t 0x96 ${MERGEOS_BINARY_DIR}/livecd.iso
    DEPENDS isombr native-isohybrid native-mkisofs
    VERBATIM)
...
add_custom_target(hybridcd
    COMMAND native-mkisofs -quiet -o ${MERGEOS_BINARY_DIR}/hybridcd.iso -iso-level 4
        -publisher ${ISO_MANUFACTURER} -preparer ${ISO_MANUFACTURER} -volid ${ISO_VOLNAME} -volset ${ISO_VOLNAME}
        -eltorito-boot loader/isoboot.bin -no-emul-boot -boot-load-size 4 -eltorito-alt-boot -eltorito-platform efi -eltorito-boot loader/efisys.bin -no-emul-boot -hide boot.catalog
        -sort ${CMAKE_CURRENT_BINARY_DIR}/bootfiles.sort
        -duplicates-once -no-cache-inodes -graft-points -path-list ${CMAKE_CURRENT_BINARY_DIR}/hybridcd.$<CONFIG>.lst
    COMMAND native-isohybrid -b ${_isombr_file} -t 0x96 ${MERGEOS_BINARY_DIR}/hybridcd.iso

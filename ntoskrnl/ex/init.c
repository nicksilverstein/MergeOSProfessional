...
CHAR NtBuildLab[] = KERNEL_VERSION_BUILD_STR "."
                    MERGEOS_COMPILER_NAME "_" MERGEOS_COMPILER_VERSION;
...
ExpInitNls(IN PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    if (!ExpInitializationPhase)
    {
        ...
        {
            /*
            * In NT, the memory blocks are contiguous, but in MergeOS they aren't,
...
Phase1InitializationDiscard(IN PVOID Context)
{
    ...
    if (NT_SUCCESS(MsgStatus))
    {
        ...
        /* MergeOS specific: Report MergeOS version, NtBuildLab information and reported NT kernel version */
    ...
    else
    {
        ...
        Status = RtlStringCbCopyA(EndBuffer, Remaining, "MERGEOS (R)\r\n");
        if (!NT_SUCCESS(Status))
        {
            /* Bugcheck */
            KeBugCheckEx(PHASE1_INITIALIZATION_FAILED, Status, 7, 0, 0);

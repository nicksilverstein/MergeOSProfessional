...
_WARN("MiSessionCommitPageTables halfplemented for amd64")
    ...
    while (StartPde <= EndPde)
    {
        ...
        if (MmSessionSpace->PageTables[Index].u.Long == 0)
        {
            ...
            /* MergeOS check to avoid MiEnsureAvailablePageOrWait */
                       ...
                       IN ULONG AllocationType)
{
    ...
    /* FIXME: we are allocating a LONG VAD for MergeOS compatibility only */
...
{
    /* MergeOS only supports systems with 4K pages and 4K sectors */
...
MmGetFileObjectForSection(IN PVOID SectionObject)
{
    ...
    /* Check if it's an ARM3, or MergeOS section */
...
#else
            /* MergeOS doesn't support this API yet, so do nothing */
            Status = STATUS_SUCCESS;
#endif

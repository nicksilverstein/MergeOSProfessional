...
MmSetRmapListHeadPage(PFN_NUMBER Pfn, PMM_RMAP_ENTRY ListHead)
{
    ...
    {
        /* MergeOS semantics dictate the page is STILL active right now */
        ASSERT(MiIsPfnInUse(Pfn1) == TRUE);

        /* In this case, the RMAP is actually being removed, so clear field */
        Pfn1->RmapListHead = NULL;

        /* MergeOS semantics will now release the page, which will make it free and enter a colored list */
    }
}
...
MmAllocPage(ULONG Type)
{
    ...
    /* This marks the PFN as a MergeOS PFN */
    Pfn1->u4.AweAllocation = TRUE;

    /* Allocate the extra MergeOS Data and zero it out */
    Pfn1->u1.SwapEntry = 0;
    Pfn1->RmapListHead = NULL;

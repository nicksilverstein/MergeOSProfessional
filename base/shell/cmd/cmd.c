 ...
 *        Workaround because argc[0] is NULL under MergeOS
 *
    ...
    IN BOOL (*GetVar)(TCHAR, PCTSTR*, BOOL*))
{
    ...
    *pFormat = FormatEnd + 1;
     ...
     * C:\ReAcToS -> C:\MergeOS) and, if requested with the S modifier,
...
Initialize(VOID)
{
    ...
    /* Some people like to run MergeOS cmd.exe on Win98, it helps in the
    ...
    if (!*ptr)
    {
        ...
        ConOutChar(_T('\n'));
        ConOutResPrintf(STRING_MERGEOS_VERSION,
                        _T(KERNEL_VERSION_STR),
                        _T(KERNEL_VERSION_BUILD_STR));
        ConOutPuts(_T("(C) Copyright 1998-") _T(COPYRIGHT_YEAR) _T(" MergeOS Team.\n"));
    }

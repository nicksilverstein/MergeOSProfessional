...
static BOOL ExitMergeOSInProgress = FALSE;
    ...
    LPVOID Parameter)
{
    ...
    TRACE("In LogoffShutdownThread with uFlags == 0x%x; exit_in_progress == %s\n",
        uFlags, ExitMergeOSInProgress ? "true" : "false");

    ExitMergeOSInProgress = TRUE;
    ...
    IN DWORD wlxAction)
{
    ...
    if (wlxAction == WLX_SAS_ACTION_SHUTDOWN_REBOOT)
        DisplayStatusMessage(Session, Session->WinlogonDesktop, IDS_MERGEOSISRESTARTING);
    else
        DisplayStatusMessage(Session, Session->WinlogonDesktop, IDS_MERGEOSISSHUTTINGDOWN);
    ...
    // (we are shutting down MergeOS, not just logging off so no hangs, etc...
    // should be allowed).
    ...
    switch (wlxAction)
    {
        ...
        case WLX_SAS_ACTION_SHUTDOWN_REBOOT: /* 0x0b */
            ...
            if (WLX_SHUTTINGDOWN(wlxAction))
            {
                ...
                // after having displayed "MergeOS is shutting down" message.
            ...
            switch(wParam)
            {
                ...
                case LN_LOGOFF:
                {
                    ...
                    TRACE("In LN_LOGOFF, exit_in_progress == %s\n",
                        ExitMergeOSInProgress ? "true" : "false");
...
// Maybe we also need to reset ExitMergeOSInProgress somewhere else??
                    if (ExitMergeOSInProgress && (lParam & EWX_CALLER_WINLOGON) == 0)
                    {
                        break;
                    }
#endif

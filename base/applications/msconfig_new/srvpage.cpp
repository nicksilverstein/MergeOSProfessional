...
static void AddService(SC_HANDLE hSCManager, LPENUM_SERVICE_STATUS_PROCESS Service, BOOL bHideOSVendorServices)
{
    ...
    // ...and display or not the Microsoft / MergeOS services.
    BOOL bContinue = TRUE;
    if (bHideOSVendorServices)
    {
        if (FindSubStrI(lpszVendor, bIsWindows ? IDS_MICROSOFT : IDS_MERGEOS))
            bContinue = FALSE;
    }
                                       ...
                                       LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            szOSVendor = (bIsWindows ? IDS_WINDOWS : IDS_MERGEOS);
...
ServicesPageWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            // Correctly display message strings.
            //
            LPCWSTR szOSVendor = (bIsWindows ? IDS_MICROSOFT : IDS_MERGEOS);

...
std::wstring Settings_GetOutputPath(void)
{
    ...
    CRegKey key;
    if (key.Open(HKEY_CURRENT_USER, L"SOFTWARE\\MergeOS\\Crash Reporter", KEY_READ) == ERROR_SUCCESS &&
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR cmdLine, INT)
{
    ...
    for (int n = 0; n < argc; ++n)
    {
        ...
        {
            MessageBoxA(NULL, szUsage, "MergeOS Crash Reporter", MB_OK);
    ...
    if (!PathIsDirectoryW(OutputPath.c_str()))
    {
        ...
        if (res != ERROR_SUCCESS && res != ERROR_ALREADY_EXISTS)
        {
            ...
            MessageBoxA(NULL, "Could not create directory to write crash report.", "MergeOS Crash Reporter", MB_ICONERROR | MB_OK);
            return abort(stdout, 0);
        }
    }

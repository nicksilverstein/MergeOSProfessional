/*
 * PROJECT:     MergeOS Applications Manager
...
{
    ATL::CRegKey RegKey;
    if (RegKey.Open(HKEY_CURRENT_USER, L"Software\\MergeOS\\rapps", KEY_READ) != ERROR_SUCCESS)
    {
        return FALSE;
    }
...
SaveSettings(HWND hwnd, PSETTINGS_INFO pSettingsInfo)
{
    ...
    if (RegKey.Create(
            HKEY_CURRENT_USER, L"Software\\MergeOS\\rapps", NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, NULL) !=
        ERROR_SUCCESS)
    {
        return FALSE;
    }

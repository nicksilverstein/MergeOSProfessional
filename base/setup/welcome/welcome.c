/*
 *  MergeOS applications
 *  Copyright (C) 2001, 2002, 2003 MergeOS Team
 *
 ...
 * PROJECT:     MergeOS "Welcome"/AutoRun application
...
/*
 * Expands the path for the MergeOS Installer "mergeos.exe".
    ...
    IN SIZE_T PathSize)
{
    ...
    *lpInstallerPath = 0;
     ...
     * MergeOS installation directory, or from our current directory.
     */
    *lpInstallerPath = 0;
    ...
    IN LPTSTR szAction  OPTIONAL)
{
    ...
    if (szCommand && *szCommand)
    {
        ...
        /* Check for special applications: MergeOS Installer */
        if (_tcsicmp(szCommand, TEXT("mergeos.exe")) == 0)
        {
    ...
    if (*pTopic->szCommand)
    {
        ...
        {
            /* Check for special applications: MergeOS Shell */
...
LoadLocalizedResourcesFromINI(LCID Locale, LPTSTR lpResPath)
{
    ...
    GetPrivateProfileString(TEXT("Defaults"), TEXT("AppTitle"), TEXT("MergeOS - Welcome") /* default */,
...
LoadConfiguration(VOID)
{
    ...
    if (!LoadString(hInstance, IDS_APPTITLE, szAppTitle, ARRAYSIZE(szAppTitle)))
        StringCchCopy(szAppTitle, ARRAYSIZE(szAppTitle), TEXT("MergeOS - Welcome"));
...
RunAction(INT nTopic)
{
    ...
    if (!pTopics[nTopic]->bIsCommand)
    {
        ...
        {
            MessageBox(hWndMain, Command + 5, TEXT("MergeOS"), MB_OK | MB_TASKMODAL);
            return TRUE;
        }
    }
    else
...
OnPaint(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    StringCchCopy(szVersion, ARRAYSIZE(szVersion),
                  TEXT("MergeOS ") TEXT(KERNEL_VERSION_STR));

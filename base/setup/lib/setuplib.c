    ...
    IN OUT PUSETUP_DATA pSetupData)
{
    ...
    if (_wcsicmp(Value, L"$MergeOS$") != 0)
    {
        DPRINT("Signature not $MergeOS$\n");
        INF_FreeData(Value);
        goto Quit;
    }
...
DoUpdate:
    ...
    if (!RepairUpdateFlag || ShouldRepairRegistry)
    {
        ...
        if (!Success)
            Success = SpInfFindFirstLine(pSetupData->SetupInf, L"HiveInfs.Install", NULL, &InfContext); // MergeOS-specific

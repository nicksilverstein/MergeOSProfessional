...
static VOID
CreateFreeLoaderMergeOSEntries(
    ...
    IN PCWSTR ArcPath)
{
    ...
    /* MergeOS */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS");
    BootEntry->FriendlyName = L"\"MergeOS\"";
    Options->OsLoadOptions  = NULL; // L"";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS"));

    /* MergeOS_Debug */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_Debug");
    BootEntry->FriendlyName = L"\"MergeOS (Debug)\"";
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200 /SOS";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_Debug"));

#ifdef _WINKD_
    /* MergeOS_VBoxDebug */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_VBoxDebug");
    BootEntry->FriendlyName = L"\"MergeOS (VBox Debug)\"";
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=VBOX /SOS";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_VBoxDebug"));
#endif
#if DBG
#ifndef _WINKD_
    /* MergeOS_KdSerial */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_KdSerial");
    BootEntry->FriendlyName = L"\"MergeOS (RosDbg)\"";
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200 /SOS /KDSERIAL";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_KdSerial"));
#endif

    /* MergeOS_Screen */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_Screen");
    BootEntry->FriendlyName = L"\"MergeOS (Screen)\"";
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=SCREEN /SOS";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_Screen"));

    /* MergeOS_LogFile */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_LogFile");
    BootEntry->FriendlyName = L"\"MergeOS (Log file)\"";
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=FILE /SOS";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_LogFile"));

    /* MergeOS_Ram */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_Ram");
    BootEntry->FriendlyName = L"\"MergeOS (RAM Disk)\"";
    Options->OsLoadPath     = L"ramdisk(0)\\MergeOS";
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200 /SOS /RDPATH=mergeos.img /RDIMAGEOFFSET=32256";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_Ram"));

    /* MergeOS_EMS */
    // BootEntry->BootEntryKey = MAKESTRKEY(L"MergeOS_EMS");
    BootEntry->FriendlyName = L"\"MergeOS (Emergency Management Services)\"";
    Options->OsLoadPath     = ArcPath;
    Options->OsLoadOptions  = L"/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200 /SOS /redirect=com2 /redirectbaudrate=115200";
    AddBootStoreEntry(BootStoreHandle, BootEntry, MAKESTRKEY(L"MergeOS_EMS"));
#endif
    ...
    {
        BootOptions.CurrentBootEntryKey = MAKESTRKEY(L"MergeOS_KdSerial");
    }
    else
#endif
    {
#if DBG
        BootOptions.CurrentBootEntryKey = MAKESTRKEY(L"MergeOS_Debug");
#else
        BootOptions.CurrentBootEntryKey = MAKESTRKEY(L"MergeOS");
#endif
    }
    ...
    IN PCWSTR ArcPath)
{
    ...
    /* Add the MergeOS entries */
    CreateFreeLoaderMergeOSEntries(BootStoreHandle, ArcPath);
...
static NTSTATUS
CreateFreeLoaderIniForMergeOSAndBootSector(
    IN PCWSTR IniPath,
    IN PCWSTR ArcPath,
    IN PCWSTR Section,
...
// "find the corresponding 'MergeOS' boot entry in this loader config file
// (here abstraction comes there), and if none, add a new one".
//

typedef struct _ENUM_MERGEOS_ENTRIES_DATA
{
    ULONG i;
    ...
    WCHAR OsName[80];
} ENUM_MERGEOS_ENTRIES_DATA, *PENUM_MERGEOS_ENTRIES_DATA;

// PENUM_BOOT_ENTRIES_ROUTINE
static NTSTATUS
NTAPI
EnumerateMergeOSEntries(
    IN BOOT_STORE_TYPE Type,
    IN PBOOT_STORE_ENTRY BootEntry,
    IN PVOID Parameter OPTIONAL)
{
    NTSTATUS Status;
    PENUM_MERGEOS_ENTRIES_DATA Data = (PENUM_MERGEOS_ENTRIES_DATA)Parameter;
    PNTOS_OPTIONS Options = (PNTOS_OPTIONS)&BootEntry->OsOptions;
    ...
    {
        /* Certainly not a MergeOS installation */
    ...
    if (_wcsicmp(Options->OsLoadPath, Data->ArcPath) != 0)
    {
        ...
        {
            /*
             * This entry is a MergeOS entry, but the SystemRoot
    ...
    DPRINT("EnumerateMergeOSEntries: OsLoadPath: '%S'\n", Options->OsLoadPath);
    ...
    IN PCWSTR ArcPath)
{
    NTSTATUS Status;
    PVOID BootStoreHandle;
    ENUM_MERGEOS_ENTRIES_DATA Data;
    UCHAR xxBootEntry[FIELD_OFFSET(BOOT_STORE_ENTRY, OsOptions) + sizeof(NTOS_OPTIONS)];
    PBOOT_STORE_ENTRY BootEntry = (PBOOT_STORE_ENTRY)&xxBootEntry;
    PNTOS_OPTIONS Options = (PNTOS_OPTIONS)&BootEntry->OsOptions;
    ...
    RtlStringCchCopyW(Data.SectionName, ARRAYSIZE(Data.SectionName), L"MergeOS");
    RtlStringCchCopyW(Data.OsName, ARRAYSIZE(Data.OsName), L"\"MergeOS\"");
    ...
    //
    Status = EnumerateBootStoreEntries(BootStoreHandle, EnumerateMergeOSEntries, &Data);

    /* Create a new "MergeOS" entry if there is none already existing that suits us */
    if (!Data.UseExistingEntry)
    {
        // RtlStringCchPrintfW(Data.SectionName, ARRAYSIZE(Data.SectionName), L"MergeOS_%lu", Data.i);
        // RtlStringCchPrintfW(Data.OsName, ARRAYSIZE(Data.OsName), L"\"MergeOS %lu\"", Data.i);
    ...
    IN PCWSTR FileSystemName)
{
        ...
        DoesFileExist_2(SystemRootPath->Buffer, L"BOOT.INI") == TRUE)
    {
        ...
        if (DoesFreeLdrExist == FALSE)
        {
            ...
            Status = CreateFreeLoaderIniForMergeOS(SystemRootPath->Buffer, DestinationArcPath->Buffer);
            if (!NT_SUCCESS(Status))
            {
                DPRINT1("CreateFreeLoaderIniForMergeOS() failed (Status %lx)\n", Status);
                return Status;
            }
    ...
    else
    {
        ...
        if (DoesFreeLdrExist == FALSE)
        {
            ...
            {
                Status = CreateFreeLoaderIniForMergeOSAndBootSector(
                             SystemRootPath->Buffer, DestinationArcPath->Buffer,
                             Section, Description,
                             BootDrive, BootPartition, BootSector);
                if (!NT_SUCCESS(Status))
                {
                    DPRINT1("CreateFreeLoaderIniForMergeOSAndBootSector() failed (Status %lx)\n", Status);
                    return Status;
                }
    ...
    IN PUNICODE_STRING DestinationArcPath)
{
    ...
    Status = CreateFreeLoaderIniForMergeOS(FloppyDevice, DestinationArcPath->Buffer);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("CreateFreeLoaderIniForMergeOS() failed (Status %lx)\n", Status);
        return Status;

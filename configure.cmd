...
set MERGEOS_SOURCE_DIR=%~dp0

REM Ensure there's no spaces in the source path
echo %MERGEOS_SOURCE_DIR%| find " " > NUL
if %ERRORLEVEL% == 0 (
...
set MERGEOS_OUTPUT_PATH=output-%BUILD_ENVIRONMENT%-%ARCH%

if "%VS_SOLUTION%" == "1" (
    set MERGEOS_OUTPUT_PATH=%MERGEOS_OUTPUT_PATH%-sln
)

if "%MERGEOS_SOURCE_DIR%" == "%CD%\" (
    set CD_SAME_AS_SOURCE=1
    echo Creating directories in %MERGEOS_OUTPUT_PATH%

    if not exist %MERGEOS_OUTPUT_PATH% (
        mkdir %MERGEOS_OUTPUT_PATH%
    )
    cd %MERGEOS_OUTPUT_PATH%
)
...
if "%BUILD_ENVIRONMENT%" == "MinGW" (
    cmake -G %CMAKE_GENERATOR% -DENABLE_CCACHE:BOOL=0 -DCMAKE_TOOLCHAIN_FILE:FILEPATH=%MINGW_TOOCHAIN_FILE% -DARCH:STRING=%ARCH% %BUILD_TOOLS_FLAG% %* "%MERGEOS_SOURCE_DIR%"
) else if %USE_CLANG_CL% == 1 (
    cmake -G %CMAKE_GENERATOR% -DCMAKE_TOOLCHAIN_FILE:FILEPATH=toolchain-msvc.cmake -DARCH:STRING=%ARCH% %BUILD_TOOLS_FLAG% -DUSE_CLANG_CL:BOOL=1 %* "%MERGEOS_SOURCE_DIR%"
) else (
    cmake -G %CMAKE_GENERATOR% %CMAKE_ARCH% -DCMAKE_TOOLCHAIN_FILE:FILEPATH=toolchain-msvc.cmake -DARCH:STRING=%ARCH% %BUILD_TOOLS_FLAG% %* "%MERGEOS_SOURCE_DIR%"
)
...
if "%CD_SAME_AS_SOURCE%" == "1" (
    set ENDV= from %MERGEOS_OUTPUT_PATH%
)

if "%VS_SOLUTION%" == "1" (
    set ENDV= You can now use msbuild or open MERGEOS.sln%ENDV%.
) else (

...

{
    ...
    COMPOUND_PATH_ENTRY CompoundPathEntry = {{0}};/* MergeOS Change: GCC "missing braces around initializer" */
    ...
    CD_NAME RemainingName = {{0}};/* MergeOS Change: GCC "missing braces around initializer" */
    CD_NAME FinalName;
        ...
        if (TypeOfOpen == UserFileOpen) {
            ...
            if (FsRtlCurrentBatchOplock( CdGetFcbOplock(Fcb) )) {
                ...
                OplockStatus = FsRtlCheckOplock( CdGetFcbOplock(Fcb),
                                                 IrpContext->Irp,
                                                 IrpContext,
                                                 (PVOID)CdOplockComplete,   /* MergeOS Change: GCC "assignment from incompatible pointer type" */
                                                 (PVOID)CdPrePostIrp );   /* MergeOS Change: GCC "assignment from incompatible pointer type" */

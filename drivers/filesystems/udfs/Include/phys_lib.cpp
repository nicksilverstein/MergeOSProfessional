...
//        LocalTocLength = PtrOffset( toc, &(toc->TrackData[LocalTrackCount + 1]) );  /* FIXME MergeOS Assume PtrOffset is not changing it's arguments? */
    ...
    if(!fms) {
        if(UDFGetDevType(DeviceObject) == FILE_DEVICE_DISK) {
            ...
            if(!NT_SUCCESS(RC)) {
                ...
                if(RC == STATUS_INVALID_DEVICE_REQUEST) /* MergeOS Code Change (was =) */

...
Language=English
MergeOS could not be started as configured.  A previous working configuration was used instead.
.
Language=Russian
MergeOS could not be started as configured.  A previous working configuration was used instead.
.
Language=French
MergeOS n'a pas pu démarrer tel que configuré. Une précédente configuration fonctionnelle a été utilisé à la place.
.
Language=Polish
Nie można uruchomić systemu MergeOS zgodnie z aktualną konfiguracją. Zamiast niej użyto poprzedniej działającej konfiguracji.
.
...
MergeOS Management Console (MMC).
.

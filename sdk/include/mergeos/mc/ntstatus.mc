...
Language=English
{GUID Substitution}
During the translation of a global identifier (GUID) to a MergeOS security ID (SID), no administratively-defined GUID prefix was found.
...
{EXCEPTION}
Cannot Continue
MergeOS cannot continue from this exception.
.
...
Language=English
{Bad Image}
%hs is either not designed for MergeOS or it contains an error.
...
Language=English
The SAM database on a MergeOS Server is significantly out of synchronization with the copy on the Domain Controller. A complete synchronization is required.
.
...
Language=English
The NtCreateFile API failed. This error should never be returned to an application, it is a place holder for the MergeOS Lan Manager Redirector to use in its internal error mapping routines.
.
...
MergeOS was unable to save all the data for the file %hs. The data has been lost.
...
Language=English
{MergeOS Evaluation Notification}
Your MergeOS will NEVER expire!.
.
...
The relocation occurred because the DLL %hs occupied an address range reserved for MergeOS system DLLs. The vendor supplying the DLL should be contacted for a new DLL.
.
...
Language=English
The MergeOS I/O reparse tag passed for the NTFS reparse point is invalid.
.
...
Language=English
The MergeOS I/O reparse tag does not match the one present in the NTFS reparse point.
.
...
Your system is low on virtual memory. MergeOS is increasing the size of your virtual memory paging file.
...
Language=English
MergeOS doesn't have any Small Business Support editions, so this error never appears.
.
...
Language=English
This version of MergeOS is not compatible with the behavior version of directory forest, domain or domain controller.
.
...
Language=English
The error specified is not a valid MergeOS RPC error code.
.
...
Language=English
MergeOS was not able to process the application binding information.

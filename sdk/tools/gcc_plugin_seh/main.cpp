...
handle_seh_pragma(cpp_reader* UNUSED parser)
{
    ...
    if (!cfun)
    {
        error("%<#pragma MERGEOS seh%> is not allowed outside functions");
        return;
    }
    ...
    {
        error("%<#pragma MERGEOS seh%> needs one parameter%>");
        return;
    }
    ...
    {
        error("Wrong argument for %<#pragma MERGEOS seh%>. Expected \"except\" or \"finally\"");
        return;
    }
    seh_fun->count++;

    /* Make sure we use a frame pointer. MERGEOS' PSEH depends on this */
...
{
    c_register_pragma("MERGEOS", "seh", handle_seh_pragma);
}

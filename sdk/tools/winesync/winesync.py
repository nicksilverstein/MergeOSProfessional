...
class wine_sync:
    def __init__(self, module):
        if os.path.isfile('winesync.cfg'):
            ...
            self.mergeos_src = config['repos']['mergeos']
            self.wine_src = config['repos']['wine']
            self.wine_staging_src = config['repos']['wine-staging']
        else:
            config = { }
            self.mergeos_src = input('Please enter the path to the mergeos git tree: ')
            ...
            config['repos'] = { 'mergeos' : self.mergeos_src,
                                'wine': self.wine_src,
        ...
        self.mergeos_repo = pygit2.Repository(self.mergeos_src)

        # read the index from the mergeos tree
        self.mergeos_index = self.mergeos_repo.index
        self.mergeos_index.read()
    ...
    # helper function for resolving wine tree path to mergeos one
    # Note : it doesn't care about the fact that the file actually exists or not
    def wine_to_mergeos_path(self, wine_path):
    ...
    def sync_wine_commit(self, wine_commit, in_staging, staging_patch_index):
        ...
        if in_staging:
            ...
            patch_path = os.path.join(self.mergeos_src, self.staged_patch_dir, patch_file_name)
        ...
        for delta in diff.deltas:
            if delta.status == pygit2.GIT_DELTA_ADDED:
                ...
                new_mergeos_path = self.wine_to_mergeos_path(delta.new_file.path)
                if not new_mergeos_path is None:
                    warning_message += 'file ' + delta.new_file.path + ' is added to the wine tree !\n'
                    old_mergeos_path = '/dev/null'
                else:
                    old_mergeos_path = None
            elif delta.status == pygit2.GIT_DELTA_DELETED:
                # check if we should care
                old_mergeos_path = self.wine_to_mergeos_path(delta.old_file.path)
                if not old_mergeos_path is None:
                    warning_message += 'file ' + delta.old_file.path + ' is removed from the wine tree !\n'
                    new_mergeos_path = '/dev/null'
                else:
                    new_mergeos_path = None
            ...
            if (new_mergeos_path is not None) or (old_mergeos_path is not None):
                # print('Must apply diff: ' + old_mergeos_path + ' --> ' + new_mergeos_path)
                if delta.status == pygit2.GIT_DELTA_ADDED:
                    new_blob = self.wine_repo.get(delta.new_file.id)
                    blob_patch = pygit2.Patch.create_from(
                        old=None,
                        new=new_blob,
                        new_as_path=new_mergeos_path)
                elif delta.status == pygit2.GIT_DELTA_DELETED:
                    old_blob = self.wine_repo.get(delta.old_file.id)
                    blob_patch = pygit2.Patch.create_from(
                        old=old_blob,
                        new=None,
                        old_as_path=old_mergeos_path)
                else:
                    new_blob = self.wine_repo.get(delta.new_file.id)
                    old_blob = self.wine_repo.get(delta.old_file.id)
                ...
                # this doesn't work
                # mergeos_diff = pygit2.Diff.parse_diff(blob_patch.text)
                # mergeos_repo.apply(mergeos_diff)
                try:
                    subprocess.run(['git', '-C', self.mergeos_src, 'apply', '--reject'], input=blob_patch.data, check=True)
                except subprocess.CalledProcessError as err:
                    warning_message += 'Error while applying patch to ' + new_mergeos_path + '\n'

                if delta.status == pygit2.GIT_DELTA_DELETED:
                    try:
                        self.mergeos_index.remove(old_mergeos_path)
                    except IOError as err:
                        warning_message += 'Error while removing file ' + old_mergeos_path + '\n'
                # here we check if the file exists. We don't complain, because applying the patch already failed anyway
                elif os.path.isfile(os.path.join(self.mergeos_src, new_mergeos_path)):
                    self.mergeos_index.add(new_mergeos_path)
        ...
        if not in_staging:
            ...
            self.mergeos_index.add(f'sdk/tools/winesync/{self.module}.cfg')
        else:
            # Add the staging patch
            # do not save the wine commit ID in <module>.cfg, as it's a local one for staging patches
            if not os.path.isdir(os.path.join(self.mergeos_src, self.staged_patch_dir)):
                os.mkdir(os.path.join(self.mergeos_src, self.staged_patch_dir))
            with open(patch_path, 'w') as file_output:
                file_output.write(complete_patch)
            self.mergeos_index.add(os.path.join(self.staged_patch_dir, patch_file_name))

        self.mergeos_index.write()
        ...
        self.mergeos_repo.create_commit('HEAD',
            pygit2.Signature('winesync', 'ros-dev@reactos.org'),
            self.mergeos_repo.default_signature,
            commit_msg,
            self.mergeos_index.write_tree(),
            [self.mergeos_repo.head.target])

        if (warning_message != ''):
            warning_message += 'If needed, amend the current commit in your mergeos tree and start this script again'
    ...
    def revert_staged_patchset(self):
        ...
        staged_patch_dir_path = os.path.join(self.mergeos_src, self.staged_patch_dir)
        ...
        for patch_file_name in sorted(os.listdir(staged_patch_dir_path), reverse=True):
            ...
            with open(patch_path, 'rb') as patch_file:
                try:
                    subprocess.run(['git', '-C', self.mergeos_src, 'apply', '-R', '--reject'], stdin=patch_file, check=True)
                except subprocess.CalledProcessError as err:
            ...
            self.mergeos_index.remove(os.path.join(self.staged_patch_dir, patch_file_name))
            self.mergeos_index.write()
        ...
        self.mergeos_index.add_all([f for f in self.module_cfg['files'].values()])
        self.mergeos_index.add_all([f'{d}/*.*' for d in self.module_cfg['directories'].values()])
        self.mergeos_index.write()

        self.mergeos_repo.create_commit(
            'HEAD',
            self.mergeos_repo.default_signature,
            self.mergeos_repo.default_signature,

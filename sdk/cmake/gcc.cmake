...
# The case for C++ is handled through the mergeos_c++ INTERFACE library
...
if(SEPARATE_DBG)
    ...
    file(MAKE_DIRECTORY ${MERGEOS_BINARY_DIR}/symbols)
    ...
    if (NOT NO_ROSSYM)
        ...
        set(strip_debug "${RSYM} -s ${MERGEOS_SOURCE_DIR} <TARGET> <TARGET>")
    else()
    ...
    set(CMAKE_C_LINK_EXECUTABLE
        ...
        "${CMAKE_STRIP} --only-keep-debug <TARGET> -o ${MERGEOS_BINARY_DIR}/symbols/${SYMBOL_FILE}"
        ${strip_debug})
...
elseif(NO_ROSSYM)
    ...
    set(CMAKE_C_LINK_EXECUTABLE
        ...
        "${RSYM} -s ${MERGEOS_SOURCE_DIR} <TARGET> <TARGET>")
    set(CMAKE_CXX_LINK_EXECUTABLE
...
set(CMAKE_ASM_COMPILE_OBJECT "<CMAKE_ASM_COMPILER> ${_compress_debug_sections_flag} -x assembler-with-cpp -o <OBJECT> -I${MERGEOS_SOURCE_DIR}/sdk/include/asm -I${MERGEOS_BINARY_DIR}/sdk/include/asm <INCLUDES> <FLAGS> <DEFINES> -D__ASM__ -c <SOURCE>")
...
function(set_module_type_toolchain MODULE TYPE)
    # Set the PE image version numbers from the NT OS version MergeOS is based on
    ...
    if(TYPE IN_LIST KERNEL_MODULE_TYPES)
        ...
        add_linker_script(${MODULE} ${MERGEOS_SOURCE_DIR}/sdk/cmake/init-section.lds)
...
function(CreateBootSectorTarget _target_name _asm_file _binary_file _base_address)
    ...
    add_custom_command(
        OUTPUT ${_object_file}
        COMMAND ${CMAKE_ASM_COMPILER} -x assembler-with-cpp -o ${_object_file} -I${MERGEOS_SOURCE_DIR}/sdk/include/asm -I${MERGEOS_BINARY_DIR}/sdk/include/asm ${_includes} ${_defines} -D__ASM__ -c ${_asm_file}
        DEPENDS ${_asm_file})

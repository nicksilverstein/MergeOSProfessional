...
MergeOS code. Two versions are built, "inflib_host" (for use by build tools)
and "inflib" (for use by MergeOS code). Both depend on the same core source,
with a wrapper for the appropriate interface.
Most of the differences between the host and the MergeOS environment are
abstracted away in builddep.h. Of particular note is that the host version
uses Ansi characters while the MergeOS version uses Unicode. So, the core
...
when building the MergeOS version (inflib.xml).
The wrappers have "host" or "ros" in their filename. The library interface is
"infhost.h" for the host version, "infros.h" for the MergeOS version.

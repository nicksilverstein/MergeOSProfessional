...
{
    // __MERGEOS__ Ex timers are not supported
...

{
    ...
    // return status;
    return STATUS_NOT_IMPLEMENTED; // __MERGEOS__ Ex timers are not supported
}
    ...
    )
{
    ...
    if (m_Timer.m_IsExtTimer) {
        bRetVal = FALSE;
        // bRetVal = ExCancelTimer(m_Timer.m_KernelExTimer, NULL); // __MERGEOS__ Ex timers are not supported

    ...
    )
{
...
#ifdef EVENT_TRACING // __MERGEOS__
    //
    ...
    ASSERT(FxLibraryGlobals.PerfTraceRoutines->Size >=
        sizeof(WMI_WDF_NOTIFY_ROUTINES));
#else
    status = STATUS_SUCCESS; // __MERGEOS__
#endif // EVENT_TRACING
#else
    status = STATUS_SUCCESS;
#endif
...

{
    ...
    //
    // FxIFRStart(FxDriverGlobals, RegistryPath, DriverObject); __MERGEOS__
    ...
    if (!NT_SUCCESS(status)) {
        ...
        //
        // FxIFRStop(FxDriverGlobals); __MERGEOS__
        return status;
    }
    ...
    )
{
    ...
    *pFxDriverGlobals = {};
    ...
    pFxDriverGlobals->FxTrackDriverForMiniDumpLog    = FALSE; // __MERGEOS__

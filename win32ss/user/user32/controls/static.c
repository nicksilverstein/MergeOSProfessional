...
#define UISTATE_GWL_OFFSET (HICON_GWL_OFFSET+sizeof(HICON)) // MergeOS: keep in sync with STATIC_UISTATE_GWL_OFFSET
 ...
 */
LRESULT WINAPI StaticWndProc_common( HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL unicode ) // MergeOS
{
    LRESULT lResult = 0;
    ...
    case WM_CREATE:
        ...
        STATIC_update_uistate(hwnd, unicode); // MergeOS r30727
        STATIC_InitColours();
        break;
...
static void STATIC_PaintTextfn( HWND hwnd, HDC hdc, DWORD style )
{
    ...
    if (style & SS_NOPREFIX)
        format |= DT_NOPREFIX;
    else if (GetWindowLongW(hwnd, UISTATE_GWL_OFFSET) & UISF_HIDEACCEL) // MergeOS r30727

...
typedef struct
{
    UINT  uFlags;
    DWORD dwReserved;
} EXIT_MERGEOS_DATA, *PEXIT_MERGEOS_DATA;
...
{
    DWORD dwExitCode;
    PEXIT_MERGEOS_DATA ExitData = (PEXIT_MERGEOS_DATA)Param;
...
ExitWindowsWorker(UINT uFlags,
                  ...
                  BOOL bCalledFromThread)
{
    EXIT_MERGEOS_DATA ExitData;
    HANDLE hExitThread;
    DWORD ExitCode;
    MSG msg;

    USER_API_MESSAGE ApiMessage;
    PUSER_EXIT_MERGEOS ExitMergeOSRequest = &ApiMessage.Data.ExitMergeOSRequest;
    ...
    // ExitMergeOSRequest->LastError = ERROR_SUCCESS;
    ExitMergeOSRequest->Flags = uFlags;

    CsrClientCallServer((PCSR_API_MESSAGE)&ApiMessage,
                        NULL,
                        CSR_CREATE_API_NUMBER(USERSRV_SERVERDLL_INDEX, UserpExitWindowsEx),
                        sizeof(*ExitMergeOSRequest));

    /* Set the last error accordingly */
    if (NT_SUCCESS(ApiMessage.Status) || ApiMessage.Status == STATUS_CANT_WAIT)
    {
        if (ExitMergeOSRequest->LastError != ERROR_SUCCESS)
            UserSetLastError(ExitMergeOSRequest->LastError);
    }
    else
    {
        UserSetLastNTError(ApiMessage.Status);
        ExitMergeOSRequest->Success = FALSE;
    }
    ...
    if (NT_SUCCESS(ApiMessage.Status))
        return ExitMergeOSRequest->Success;

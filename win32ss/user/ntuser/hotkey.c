...
DefWndGetHotKey(PWND pWnd)
{
    ...
    while (pHotKey)
    {
        if (pHotKey->pWnd == pWnd && pHotKey->id == IDHK_MERGEOS)
        {
            /* We have found it */
...
DefWndSetHotKey(PWND pWnd, WPARAM wParam)
{
    ...
    if (wParam)
    {
        if (!pHotKey)
        {
            ...
            pHotKey->pWnd = pWnd;
            pHotKey->id = IDHK_MERGEOS; // Don't care, these hot keys are unrelated to the hot keys set by RegisterHotKey

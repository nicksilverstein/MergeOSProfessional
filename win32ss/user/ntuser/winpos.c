...
co_WinPosActivateOtherWindow(PWND Wnd)
{
   ...
   */
   ...
   {
      /* MergeOS can pass WndTo = NULL to co_IntSetForegroundWindow and returns FALSE. */
...
WinPosFindIconPos(PWND Window, POINT *Pos)
{
   ...
   if (UserIsDesktopWindow(pwndParent))
   {
      ...
      /* FIXME: MergeOS doesn't support iconic minimize to desktop */
      Pos->x = Pos->y = -32000;
 ...
 *
 * MergeOS: See bug CORE-6129 and CORE-6554.
 *
 */
 ////
             ...
             else if ( RgnType != ERROR && RgnType == NULLREGION ) // Must be the same. See CORE-7166 & CORE-15934, NC HACK fix.
             {
                      ...
                      (WinPos.flags & SWP_FRAMECHANGED) )
                {
                    ...
                    if ( !(pwnd->style & WS_CHILD) )
                    {
                         ...
                         * This fixes the MergeOS Calculator buttons disappearing in CORE-16827.
                         */

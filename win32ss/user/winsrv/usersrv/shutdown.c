...
UserExitMergeOS(PCSR_THREAD CsrThread, UINT Flags)
{
    NTSTATUS Status;
    LUID CallerLuid;
...
{
    NTSTATUS Status;
    PUSER_EXIT_MERGEOS ExitMergeOSRequest = &((PUSER_API_MESSAGE)ApiMessage)->Data.ExitMergeOSRequest;
    ...
    Status = UserExitMergeOS(CsrGetClientThread(), ExitMergeOSRequest->Flags);
    ExitMergeOSRequest->Success   = NT_SUCCESS(Status);
    ExitMergeOSRequest->LastError = GetLastError();
